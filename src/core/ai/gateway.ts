...
export function isAvailable(touchpoint: TouchpointKind): boolean {
  ...
  try {
    ...
    const touchpointConfig =
      recipe.touchpoints[touchpoint as "embedding" | "expansion" | "chat"];
...
function instantiateEmbedding(
  ...
  switch (recipe.implementation) {
    ...
    default:
      throw new AIConfigError(
        `Unknown implementation: ${(recipe as any).implementation}`,
      );
  }
}
...
export async function embed(
  ...
  const cfg = requireConfig();
  const { model, recipe, modelId } = await resolveEmbeddingProvider(
    modelOverride ?? getEmbeddingModel(),
  );
...
function effectiveSafetyFactor(recipe: Recipe): number {
  const declared =
    recipe.touchpoints?.embedding?.safety_factor ?? DEFAULT_SAFETY_FACTOR;
 ...
 * @internal exported for tests; not part of the public gateway API.
 */
export function __getShrinkStateForTests(
  recipeId: string,
): ShrinkEntry | undefined {
...
export async function embedMultimodal(
  ...
  if (
    touchpoint.multimodal_models &&
    !touchpoint.multimodal_models.includes(parsed.modelId)
  ) {
    throw new AIConfigError(
  ...
  for (let i = 0; i < inputs.length; i += MULTIMODAL_BATCH_SIZE) {
    ...
    try {
      ...
      throw normalizeAIError(
        err,
        `embedMultimodal(${recipe.id}:${parsed.modelId})`,
      );
    }
    ...
    for (const row of parsedBody.data) {
      if (
        ...
        row.embedding.length !== targetDims
      ) {
        throw new AIConfigError(
...
function instantiateExpansion(
  ...
  switch (recipe.implementation) {
    case "native-openai": {
      ...
      if (!apiKey)
        throw new AIConfigError(
          `OpenAI expansion requires OPENAI_API_KEY.`,
          recipe.setup_hint,
        );
...
export async function expand(query: string): Promise<string[]> {
  ...
  try {
    const { model, recipe, modelId } =
      await resolveExpansionProvider(getExpansionModel());
  ...
  switch (recipe.implementation) {
    ...
    case "openai-compatible": {
      ...
      if (required.length > 0 && !apiKey) {
        throw new AIConfigError(
          `${recipe.name} requires ${required[0]}.`,
          recipe.setup_hint,
        );
      }
      return createOpenAICompatible({
...
function mapStopReason(
  ...
  const anthropicStop =
    providerMetadata?.anthropic?.stopReason ??
    providerMetadata?.anthropic?.stop_reason;
